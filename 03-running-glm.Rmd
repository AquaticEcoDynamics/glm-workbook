...
- Use the `cd` (change directory) command to navigate to the simulation directory (e.g. the *Kinneret97* folder)
...
These steps are shown in the screen recording below for the *Kinneret97* simulation:
...
On Windows, the model can be run using a batch-file `glm.bat`, which you can open from in the simulation directory (e.g. *Kinneret97* folder). The `.bat` file calls the GLM model from the current working directory.
