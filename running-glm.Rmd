...
On Windows, the model can be run using a batch-file `run_glm.bat`, which you can open from in the simulation directory (e.g. _warmlake_ folder). The `.bat` file calls the GLM model from the current working directory, and should be located where the `glm.nml' file is.
...
### Running and editing GLM with Visual Studio Code _Batch Runner_ extenstion {#C02_5 -}
...
- open the glm.bat batch-file, click the _Run_ symbol (see figure below) to run GLM
